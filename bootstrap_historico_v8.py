...
# True = limpia el SQLite antes de reconstruir (recomendado)
...
def init_sqlite(ruta_db: str, reset: bool):
    """Crea las tablas con el esquema V8. Si reset=True, borra primero."""
...
def main():
    ...
    if not sol_folder:
        ...
        # Todos los correos (sin filtro UnRead) desde FECHA_INICIO
        todos = []
        ...
        for m in todos:
            try:
                ...
                # Saltar si ya está en SQLite
                if existe_conversation(RUTA_DB_SQLITE, conv_id):
                    duplicados_sol += 1
                    continue

                # Parsear cuerpo (modo tolerante para datos históricos:
                # no filtramos por campos requeridos, campos vacíos = "-")
                ...
                # Guardar .msg (solo si no existe ya)
        ...
        for m in todos_resp:
            try:
                ...
                if not existe_conversation(RUTA_DB_SQLITE, conv_id):
                    # Respuesta sin solicitud registrada en el rango
                    continue
