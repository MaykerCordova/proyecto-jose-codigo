...
class GestorBackupSQL:
    ...
    def existe_conversation(self, conv_id: str) -> bool:
        """
        Deduplicación por ConversationID — automático, sin regex.
        No depende de que el Maker escriba un [ID] en el asunto.
        """
        with sqlite3.connect(self.ruta_db) as conn:
            r = conn.execute(
    ...
    def actualizar_aprobacion(
        ...
        """
        V8: actualiza también Sustento_Respuesta con la ruta del .msg
        de la conformidad, igual que Sustento guarda la solicitud.
        """
        with sqlite3.connect(self.ruta_db) as conn:
            conn.execute("""
                UPDATE Bitacora
...
def exportar_excel_desde_sqlite(sql: GestorBackupSQL, ruta: str):
    """
    El Excel es solo una vista — se regenera completo desde SQLite.
    ConversationID se incluye al final como referencia técnica.
    """
    try:
...
class ProcesadorTexto:
    ...
    def extraer_id(asunto: str):
        """
        Extrae el [ID] del asunto si existe — solo para referencia.
        Ya no es la llave primaria en V8.
        """
...
def main():
    ...
    if not sol_folder:
        ...
        for m in msgs:
            try:
                # ── Llave principal: ConversationID automático ─────
                conv_id = m.ConversationID

                # ── ID del asunto (solo referencia) ────────────────
    ...
    if not resp_folder:
        ...
        for m in msgs:
            try:
                # ── V8: match directo por ConversationID ───────────
                # Sin regex, sin depender del asunto
                conv_id      = m.ConversationID
                ...
                # ── Guardar .msg respuesta y capturar ruta ─────────
